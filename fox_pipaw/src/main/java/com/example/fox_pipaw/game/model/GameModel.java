...
public class GameModel implements IGameModel {
    ...
    public void queryGiftList(final IRequestCallback callback) {
        AsyncTaskTool.load(HttpUrl.recomondUrl).execute(new AsyncTaskTool.IMyCallback() {
            @Override
            public void success(String result) {
                List<NewAndHotBean> datas = new ArrayList<NewAndHotBean>();
                try {
                    ...
                    for (int i = 0; i < length; i++) {
                        ...
                        NewAndHotBean newAndHotBean = new NewAndHotBean(descript, fahao_count,
                                game_id, game_logo, game_name, game_visits, size,game_type);
